...
public class NumberOfDiscIntersectionsChallenge {
    ...
    public static int solution(int[] A) {
        ...
        for (int i = 0; i < A.length; i++) {
            ...
            // If firstRadius exceeds the length of the array it intersects all following radius's. Therefore,
            // add total number of remaining indexes to numberOfIntersections and skip second loop.
            if (firstRadius >= A.length - indexTracker) {
                numberOfIntersections += (A.length - indexTracker);

            } else {
                numberOfIntersections += firstRadius;
                for (int j = indexTracker + firstRadius; j < A.length; j++) {
                    int secondRadius = A[j];
                    distanceBetweenPoints++;

                    if ((long) distanceBetweenPoints - secondRadius <= 0) {
