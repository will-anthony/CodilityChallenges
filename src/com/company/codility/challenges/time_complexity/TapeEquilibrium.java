...
//    each element of array A is an integer within the range [−1,000..1,000].
    ...
    public static int solution(int[] A) {
        ...
        int sumOfSecondArray = addAllElements(A);
        int smallestDifference = Integer.MAX_VALUE;

        for (int i = 0; i < A.length - 1; i++) {

            sumOfFirstArray += A[i];
            sumOfSecondArray -= A[i];

            int potentialSmallestDifference = Math.abs(sumOfFirstArray - sumOfSecondArray);

            if (potentialSmallestDifference < smallestDifference) {
                smallestDifference = potentialSmallestDifference;
            }
        }
